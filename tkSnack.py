...
class SoundFrame(tk.Frame):
    ...
    def __init__(self, parent=None, sound=None, *args, **kw):
        ...
        bbar = tk.Frame(self)
        bbar.pack(side='left')
        tk.Button(bbar, image='snackOpen', command=self.load
                       ).pack(side='left')
        tk.Button(bbar, bitmap='snackPlay', command=self.play
                       ).pack(side='left')
        tk.Button(bbar, bitmap='snackRecord', fg='red',
                       command=self.record).pack(side='left')
        tk.Button(bbar, bitmap='snackStop', command=self.stop
                       ).pack(side='left')
        tk.Button(bbar, text='Info', command=self.info).pack(side='left')
...
class SnackCanvas(tk.Canvas):
    
    def __init__(self, master=None, cnf={}, **kw):
        tk.Widget.__init__(self, master, 'canvas', cnf, kw)
